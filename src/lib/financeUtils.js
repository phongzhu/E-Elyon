...
export function aggregate(transactions) {
  ...
  for (const t of transactions || []) {
    ...
    if (t.type === "Income" && !isTransfer) out.income += amt;
    if (t.type === "Expense" && !isTransfer) out.expenses += amt;

    if (category === "tithe" || category === "tithes") out.tithes += amt;
    if (category === "offering" || category === "offerings") out.offerings += amt;
    if (["special donation", "donation", "donations", "gift", "gifts"].includes(category)) {
      out.donations += amt;
    }

    if (t.type === "Income" && !isTransfer && !isDonationCategory) {
      out.otherIncome += amt;
    }
  }
  out.donationTotal = out.tithes + out.offerings + out.donations;
