...
import { fetchBranches } from "../../lib/financeAccountsService";
...
export default function FinanceDashboard() {
  ...
  const loadUserBranch = async () => {
    try {
      ...
      setSelectedBranches([detailsData.branch_id]);
    } catch (error) {
  ...
  const loadData = async () => {
    ...
    try {
      const branchFilter = selectedBranches.length > 0 ? selectedBranches : null;

      let accountsQuery = supabase
        .from("finance_accounts")
        .select("account_id, account_name, account_type, is_active, branch_id, branches:branch_id(name)")
        .is("branch_ministry_id", null)
        .eq("is_active", true);

      if (branchFilter) {
        accountsQuery = accountsQuery.in("branch_id", branchFilter);
      }

      // Load accounts for selected branches (no ministry only)
      ...
      const accountBranchMap = new Map(
        (accountsData || []).map((a) => [
          a.account_id,
          {
            name: a.branches?.name || "Unknown Branch",
      ...
      if (accountIds.length > 0) {
        ...
        const normalized = (txData || []).map((t, i) => {
          ...
          const branchInfo = accountBranchMap.get(t.account_id) || {};
          return {
            id: t.transaction_id ?? i + 1,
            date: t.transaction_date ?? new Date(),
            type: txType,
            category: txCategory,
            branch: branchInfo.name || userBranchName,
  ...
  const branchLabel = useMemo(() => {
    if (!branches.length) {
      return selectedBranches.length > 0 ? "Selected Branches" : "All Branches";
    }
    const names = branches
      .filter((b) => selectedBranches.includes(b.branch_id))
      .map((b) => b.name)
      .filter(Boolean);
    if (names.length === 0) return "All Branches";
          ...
          <div className="max-w-7xl mx-auto">
            <div className="mb-8">
              ...
              <p className="text-gray-600 mt-2">
                Financial overview for <span className="font-semibold" style={{ color: primaryColor }}>{branchLabel}</span>
              </p>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              ...
              <div className="relative">
                ...
                {showBranchFilter && (
                  <div className="absolute left-0 mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-10 max-h-96 overflow-y-auto">
