...
export default function PastorUserManagement() {
  ...
  const fetchEligibleMembers = async (branchId, { silent = false } = {}) => {
    try {
      ...
      const mapped = (data || []).map((m) => {
        ...
        const preferred =
          accounts.find((a) => /_member(?=@)/i.test(a.email) || /_member$/i.test(a.email)) ||
          accounts[0];
  ...
  const handleCreateAccountForMember = async () => {
    try {
      ...
      if (String(selectedMember.branch_id) !== String(pastorCtx.branch_id)) {
        return setErr("You can only create accounts for members in your assigned branch.");
      }
      ...
      const allowedRole = ["FINANCE", "STAFF"].includes(createForm.role) ? createForm.role : "STAFF";
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
        ...
        <main className="p-6 md:p-8 space-y-6 w-full">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div className="space-y-1">
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Pastor User Management</h1>
              ...
              <div className="inline-flex items-center gap-2 mt-2 text-xs">
                ...
                <span className="text-gray-700 font-medium">{pastorCtx.branch_name || "—"}</span>
              </div>
            </div>
              ...
              {mainTab === "add-user" && (
                <div className="space-y-6">
                  <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-sm text-amber-900 flex items-start gap-2">
                    ...
                    <div>
                      <div className="font-semibold">Add User (Pastor View)</div>
                      <div className="text-amber-800">
                        Only baptized members within your branch can be selected. You cannot assign access to yourself.
                      </div>
                    </div>
                  </div>
                        ...
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                          <div className="w-full md:w-2/3">
                            <label className="block text-sm font-semibold mb-2 text-gray-800">
                              Search Baptized Members (Branch: {pastorCtx.branch_name || "—"})
                            </label>
                            <input
                              value={searchTerm}
                                  ...
                                  <tr key={m.user_details_id} className="border-t hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-center">
                                      <button
                                        ...
                                        title={disabledSelf ? "You cannot select yourself." : "Select member"}
                                      >
                                        Select
                                      </button>
                                    </td>
                    ...
                    <div className="lg:col-span-5">
                      <div className="bg-white border border-gray-100 rounded-2xl p-5 md:p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                          <h4 className="font-bold text-lg text-gray-900">Selected Member & Create Account</h4>
                          {selectedMember && (
                            <button
                        ...
                        {!selectedMember ? (
                          <div className="p-8 text-center text-gray-500 border-2 border-dashed rounded-2xl">
                            Select a baptized member from the list to create an account.
                          </div>
                        ) : selectedMember.is_self ? (
                          <div className="p-6 rounded-2xl bg-rose-50 border border-rose-200 text-rose-800 text-sm">
                            You selected yourself. Pastors cannot assign access roles to themselves.
                          </div>
                        ) : (
                          <div className="space-y-5">
                            <div className="grid grid-cols-1 gap-3">
                              <InfoBox label="Full Name" value={selectedMember.full_name} />
                              <InfoBox label="Contact Number" value={selectedMember.contact_number || "-"} />
                              <InfoBox label="Member Email" value={selectedMember.display_member_email || "-"} />
                              <InfoBox label="Branch" value={pastorCtx.branch_name || "-"} />
                            ...
                            <div className="grid grid-cols-1 gap-4">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  <label className="block text-sm font-semibold mb-2">Access Start Date *</label>
                                  <input
                                    type="date"
                                ...
                                <div>
                                  <label className="block text-sm font-semibold mb-2">Access End Date *</label>
                                  <input
                                    type="date"
                              ...
                              {!selectedMember.preferred_auth_user_id && (
                                <div className="text-xs text-rose-700 bg-rose-50 border border-rose-200 p-3 rounded-xl">
                                  This member has no linked Auth account. They must exist in Supabase Auth first.
                                </div>
                              )}
                            </div>
                          </div>
      ...
      <UserDetailsModal user={drawerUser} onClose={() => setDrawerUser(null)} stripEmail={stripMemberSuffixEmail} />
    </div>
  );
}
...
function InfoBox({ label, value, mono }) {
  return (
    <div>
      ...
      <div className={`p-3 border rounded-xl bg-gray-50 ${mono ? "font-mono text-xs" : ""}`}>
        {value || "-"}
      </div>
    </div>
  );
        ...
        <div className="p-6 space-y-5">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <div className="text-xl font-bold text-gray-900 truncate">{user.full_name}</div>
              <div className="text-sm text-gray-600 truncate">{stripEmail(user.email) || "-"}</div>
            </div>
          </div>
        </div>
      </div>
