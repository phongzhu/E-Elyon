...
import { aggregate, filterTransactions, BRANCHES, formatCurrency } from "../../lib/financeUtils";
...
export default function PastorDashboard() {
	...
	// Get branch from localStorage (set at login)
	useEffect(() => {
		setBranch(localStorage.getItem("branchId"));
	}, []);
	...
	useEffect(() => {
		...
		(async () => {
			try {
				// Only fetch data for the assigned branch
				const { data, error } = await supabase.from("finance_transactions").select("*").eq("branch_id", branch);
				const transactions = error || !data || !data.length ? [] : data.map((t, i) => ({
					id: t.id ?? i + 1,
				...
				const { data: attData } = await supabase.from("attendance").select("date, branch, count").eq("branch", branch);
				...
				const { data: counselData } = await supabase.from("counseling_requests").select("id, member_name, request_type, status, created_at, branch").eq("branch", branch);
				...
				if (mounted) {
					...
					setCounselingRequests(counselData || []);
				}
			} catch (_) {
				if (mounted) {
					setTx([]);
	...
	const totalMembers = attendanceSummary.reduce((sum, row) => sum + Number(row.count || 0), 0) || 0;
	const volunteersCount = (workers || []).length || 0;
	const workersCount = (ministries || []).length * 5 || 0;
				...
				<main className="flex-1 p-6 overflow-y-auto">
						...
						<div className="flex items-center justify-between gap-4 flex-wrap">
							<div>
								...
								<p className="text-sm text-gray-600">Branch-level view of members, finances, and events.</p>
							</div>
						...
						<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
							<div className="space-y-6 lg:col-span-2">
								...
								<div className="bg-white rounded-lg shadow-sm border border-gray-100 p-5">
									<h3 className="text-base font-semibold text-gray-900 mb-3">Counseling Requests</h3>
...
// --- Reuse the same stat, analytics, calendar, and event card components as BishopDashboard ---
					...
					<div>
						<div className="flex items-center justify-between mb-4">
							<div>
								...
								<div className="text-2xl font-bold text-gray-900">{formatCurrency(totalFunds || 0)}</div>
